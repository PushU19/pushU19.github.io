if (!window.loaded_modss) {
    ...
    (function() {
        ...
        var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
        var version_modss = '3.4', API = Protocol() + 'api.modss.tv/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = false, leftVipD = '', user_id = 142486046, uid = 'bb04b81f6297b1b6280ea8273_142486046', IP, logged = true, cashe = encodeURIComponent(Lampa.Base64.encode(window.location.origin));
        ...
        console.log('Modss', 'init', 'LOADED - DE [85.118.165.219] - [GET] ' + urls[0]);
        ...
        function tryNextUrl() {
            if (currentUrlIndex >= urls.length) {
                ...
                console.log('Modss', 'init', 'DE [85.118.165.219] - все URL недоступны. Статусы ответов:', statusCodes, loadErrors);
                Lampa.Noty.show('DE [85.118.165.219] - MODSs ОШИБКА ЗАГРУЗКИ ПЛАГИНА -> все серверы недоступны.<br>Статусы: ' + (statusCodes || '—') + '<br>Lampa: ' + window.location.origin, {
                    time: 10000
                });
            ...
            network.silent(currentUrl, function(json) {
                ...
                console.log('Modss', 'init', 'DE [85.118.165.219] - load get plugin error - [POST] ' + currentUrl, 'status:', statusNum, errMsg);
                if (statusNum === 444 || statusNum === 403) {
                    console.error('Modss', 'init', 'ERROR - BLOCKED (status ' + statusNum + ')');
                    Lampa.Noty.show('MODSs BLOCKED — ' + (errMsg || statusNum), {
                        time: 8000
                    });
                    window.loaded_modss = false;
                    return;
